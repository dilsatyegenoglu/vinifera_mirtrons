...
BASE_IN = Path("blast_results")
BASE_OUT = Path("drosha_free_results")
...
def analyze_overhang_distribution(rows, max_overhang):
    """Analyze overhang distribution patterns"""
    o5_values = []
    o3_values = []
    ...
    for r in rows:
        try:
            o5 = float(r.get("O5", 999))
            o3 = float(r.get("O3", 999))
